...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
            ...
            <form method="GET" class="relative w-full" id="searchForm">
                <input type="text" name="search" value="{{ request('search') }}"
                   ...
                   focus:border-blue-500 focus:ring-blue-500"
                    placeholder="search mata pelajaran..." id="searchInput">

                <input type="hidden" name="dataMapel" value="{{ request('dataMapel') }}">
        ...
        <div class="w-full sm:w-auto">
            <button type="button" onclick="location.href='{{ route('admin.mapel.create') }}'"3
                ...
                <div class="overflow-hidden border border-gray-200 rounded-lg">
                    ...
                    sm:flex sm:items-center sm:justify-between">
                        ...
                        <nav class="flex justify-center sm:justify-start items-center gap-x-1" aria-label="Pagination">
                            ...
                            <!-- Previous -->
                            <a href="{{ $dataMapel->previousPageUrl() ?? '#' }}"
                                ...
                                {{ $dataMapel->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                            ...
                            <div class="flex items-center gap-x-1">
                                @foreach ($dataMapel->getUrlRange(max(1, $dataMapel->currentPage() - 2), min($dataMapel->lastPage(), $dataMapel->currentPage() + 2)) as $page => $url)
                                    <a href="{{ $url }}"
                                        class="min-h-9.5 min-w-9.5 flex justify-center items-center
                                        py-2 px-3 text-sm rounded-lg
                                        {{ $page == $dataMapel->currentPage()
                            ...
                            <!-- Next -->
                            <a href="{{ $dataMapel->nextPageUrl() ?? '#' }}"
                                class="min-h-9.5 min-w-9.5 py-2 px-2.5 inline-flex justify-center items-center
                                text-sm rounded-lg border border-transparent text-gray-800
                                hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100
                                {{ $dataMapel->hasMorePages() ? '' : 'opacity-50 pointer-events-none' }}">
                        ...
                        <div
                            ...
                            <div class="hs-dropdown-menu hs-dropdown-open:opacity-100 w-48 hidden z-50
                                ...
                                dataMapel="menu" aria-labelledby="hs-bordered-pagination-dropdown">
