...
@section('content')
    <div class="grid grid-cols-1 gap-6">
        <div class="bg-white border border-gray-200 shadow-2xs rounded-xl p-6">
            ...
            <form action="{{ route('admin.guru_mapel.update', $guruMapel->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select id="user_id" name="user_id"
                        class="select2 py-3 px-4 pe-9 block w-full rounded-lg text-sm
                        border {{ $errors->has('user_id')
                            ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                            : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500' }}">

                        <option value="" disabled {{ old('user_id', $guruMapel->user_id) ? '' : 'selected' }}>
                            Pilih Guru
                        </option>

                        @foreach ($guru as $g)
                            <option value="{{ $g->id }}"
                                {{ old('user_id', $guruMapel->user_id) == $g->id ? 'selected' : '' }}>
                                {{ $g->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="mata_pelajaran_id" name="mata_pelajaran_id"
                        class="select2 py-3 px-4 pe-9 block w-full rounded-lg text-sm
                        border {{ $errors->has('mata_pelajaran_id')
                            ? 'border-red-500 focus:border-red-500 focus:ring-red-500'
                            : 'border-gray-200 focus:border-blue-500 focus:ring-blue-500' }}">

                        <option value="" disabled
                            {{ old('mata_pelajaran_id', $guruMapel->mata_pelajaran_id) ? '' : 'selected' }}>
                            Pilih Mata Pelajaran
                        </option>

                        @foreach ($mapel as $m)
                            <option value="{{ $m->id }}"
                                {{ old('mata_pelajaran_id', $guruMapel->mata_pelajaran_id) == $m->id ? 'selected' : '' }}>
                                {{ $m->nama_mapel }}
                            </option>
                        @endforeach
                    </select>
