...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full sm:max-w-lg">
            ...
            <form method="GET" class="relative w-full" id="searchForm">
                <input type="text" name="search" value="{{ request('search') }}"
                    ...
                    placeholder="search guru..." id="searchInput">

                <input type="hidden" name="guru" value="{{ request('guru') }}">
            ...
            <!-- Filter -->
            @php
                $activeMapel = $mapels->firstWhere('id', request('mapel'));
            @endphp
            ...
            <div class="hs-dropdown [--auto-close:inside] relative inline-flex w-full sm:w-auto">
                ...
                <button id="hs-dropdown-default" type="button"
                    ...
                    {{ $activeMapel ? $activeMapels->nama_mapel : 'Filter' }}
        ...
        <div class="w-full sm:w-auto">
            <button type="button" onclick="location.href='{{ route('admin.guru_mapel.create') }}'"3
                ...
                <div class="overflow-hidden border border-gray-200 rounded-lg">
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-end text-sm font-medium">
                                        <div class="inline-flex items-center gap-2">
                                            {{-- Detail --}}
                                            <a href="{{ route('admin.guru_mapel.show', $item->id) }}"
                    ...
                    <div
                        ...
                        <nav class="flex justify-center sm:justify-start items-center gap-x-1" aria-label="Pagination">
                            ...
                            <!-- Previous -->
                            <a href="{{ $dataGuruMapel->previousPageUrl() ?? '#' }}"
                                ...
                                {{ $dataGuruMapel->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
                            ...
                            <div class="flex items-center gap-x-1">
                                @foreach ($dataGuruMapel->getUrlRange(max(1, $dataGuruMapel->currentPage() - 2), min($dataGuruMapel->lastPage(), $dataGuruMapel->currentPage() + 2)) as $page => $url)
                                    <a href="{{ $url }}"
                                        class="min-h-9.5 min-w-9.5 flex justify-center items-center
                                        py-2 px-3 text-sm rounded-lg
                                        {{ $page == $dataGuruMapel->currentPage()
                            ...
                            <!-- Next -->
                            <a href="{{ $dataGuruMapel->nextPageUrl() ?? '#' }}"
                                class="min-h-9.5 min-w-9.5 py-2 px-2.5 inline-flex justify-center items-center
                                text-sm rounded-lg border border-transparent text-gray-800
                                hover:bg-gray-100 focus:outline-hidden focus:bg-gray-100
                                {{ $dataGuruMapel->hasMorePages() ? '' : 'opacity-50 pointer-events-none' }}">
