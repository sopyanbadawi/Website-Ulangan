...
@section('content')
    <div class="grid grid-cols-1 gap-6">
        <div class="bg-white border border-gray-200 shadow-2xs rounded-xl p-6">
            ...
            <form action="{{ route('admin.tahun.update', $tahunAjaran->id) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    border {{ $errors->has('semester')
                        ...
                        <option value="ganjil"
                            {{ strtolower(old('semester', $tahunAjaran->semester)) == 'ganjil' ? 'selected' : '' }}>
                            Ganjil
                        </option>

                        <option value="genap"
                            {{ strtolower(old('semester', $tahunAjaran->semester)) == 'genap' ? 'selected' : '' }}>
                            Genap
                        </option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" id="tahun" name="tahun" value="{{ old('tahun', $tahunAjaran->tahun) }}"
