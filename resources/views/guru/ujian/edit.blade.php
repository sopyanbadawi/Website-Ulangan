...
@section('content')
    <div class="grid grid-cols-1 gap-6 max-w-5xl space-y-4">
        ...
        <form id="form-ujian" action="{{ route('guru.ujian.update', $ujian->id) }}" method="POST"
                        ...
                        <div class="soal-item border rounded-xl p-4">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="soal[{{ $i }}][pertanyaan]"
                                    class="py-2.5 sm:py-3 px-4 block w-full border-gray-200 rounded-lg sm:text-sm
                                    focus:border-blue-500 focus:ring-blue-500
                                    dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400">{{ $soal->pertanyaan }}</textarea>
                            </div>
                                ...
                                @foreach ($soal->opsiJawaban as $j => $opsi)
                                    <div class="opsi-item border rounded-xl p-3 flex gap-3 items-start">
                                        ...
                                        <div class="flex-1 space-y-3">
                                            ...
                                            @if ($opsi->opsi_gambar)
                                                <img src="{{ asset($opsi->opsi_gambar) }}" class="h-20 rounded">
                                            @endif
    ...
    <script>
        ...
        function hitungTotalBobot() {
            ...
            document.querySelectorAll('.bobot-input').forEach(i => {
                totalBobot += parseInt(i.value) || 0;
            });

            const sisa = maxBobot - totalBobot;
            document.getElementById('sisa-bobot').innerText = `Sisa Bobot: ${sisa}`;

            document.getElementById('btn-tambah-soal').disabled = sisa <= 0;
        }
        ...
        function addSoal() {
            ...
            const html = `
<div class="soal-item border rounded-xl p-4 mb-4" data-index="${soalIndex}">
    ...
    `;
        ...
        function hapusSoal(btn) {
            ...
            Swal.fire({
                ...
                // 🔥 SOAL DARI DATABASE
                // =========================
                fetch(`{{ route('guru.ujian.soal.destroy', ':id') }}`.replace(':id', soalId), {
                        method: 'DELETE',
                        headers: {
        ...
        function addOpsi(btn, idx) {
            ...
            const opsiIndex = wrapper.children.length;
...
<div class="opsi-item border rounded-xl p-3 flex gap-3 items-start">
    ...
    <input
        ...
        value="${opsiIndex}"
        required
        class="mt-2 shrink-0 border-gray-200 text-blue-600
               focus:ring-blue-500
    ...
    <div class="flex-1 space-y-3">
        ...
        <div>
            ...
            <input
                type="text"
                name="soal[${idx}][opsi][${opsiIndex}][teks]"
                placeholder="Masukkan teks opsi ${opsiIndex + 1}"
                class="py-2.5 sm:py-3 px-4 block w-full border-gray-200 rounded-lg sm:text-sm
                       focus:border-blue-500 focus:ring-blue-500
                       dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400
                       dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
        ...
        <div>
            <label class="block text-sm font-medium mb-1 dark:text-white">
                Gambar Opsi ${opsiIndex + 1} (opsional)
            </label>
            <input
                type="file"
                name="soal[${idx}][opsi][${opsiIndex}][gambar]"
                class="block w-full text-sm text-gray-500
                       file:me-4 file:py-2 file:px-4
                       file:rounded-lg file:border-0
                       file:text-sm file:font-semibold
...
<div class="flex items-end gap-3 mt-3">
    ...
    `);
        ...
        /* ================= VALIDASI SUBMIT ================= */
        document.getElementById('form-ujian').addEventListener('submit', e => {
            if (totalBobot !== 100) {
                e.preventDefault();
